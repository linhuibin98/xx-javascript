...
let list: number[] = [123, 1, 3];  // 由数字组成的数组

// 第二种方式是使用数组泛型，Array<元素类型>

let arr: Array<number> = [2, 3, 4];  // 不推荐使用

// 元组 Tuple

// 元组类型允许表示一个已知元素数量和类型的数组，各元素的类型不必相同。 比如，你可以定义一对值分别为 string和number类型的元组

let x: [string, number];   // 数组x中只能存在字符串和数字类型，而且前两位必须对应 字符串，数字
x = ['hello', 18];
...
let h: number = (g as string).length;
